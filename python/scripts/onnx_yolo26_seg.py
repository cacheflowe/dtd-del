...
# Frames to linearly ramp a track's Presence value 0->1 on first fresh detection, and
# 1->0 once it stops being freshly detected (lost_frames > 0) -- see
# object_tracker.presence_ramp(). Independent of Tracklossframes/track_buffer: this is
# about how long the OUTPUT should visually fade, not how long the tracker keeps
# hunting for a re-match. Keep PRESENCE_FALL_FRAMES <= Tracklossframes so Presence
# actually reaches 0 before the track itself is pruned.
PRESENCE_RISE_FRAMES = 5
PRESENCE_FALL_FRAMES = 15
...
class YOLO26SegmentationInference(ONNXInferenceManager):
	...
	def __init__(self):
		...
		self._mask_state = {}
		# Per-track Presence ramp state (see object_tracker.presence_ramp()), keyed by
		# track_id -- a smooth [0,1] found/lost scalar for driving an effect's scale/
		# opacity, independent of the score-threshold display gate below (which reacts
		# to noisy raw per-frame confidence) and independent of track_buffer (which
		# governs tracker-internal re-match grace, not visual fade duration).
	...
	def onSetupParameters(self, scriptOp):
		...
		p = page.appendFloat('Presencerise', label='Presence Rise Frames', size=1)
		p[0].default = PRESENCE_RISE_FRAMES
		p[0].min = 1.0
		p[0].max = 60.0
		p[0].clampMin = True
		p[0].clampMax = False
		p[0].help = object_tracker.par_help('Presencerise', subject='person')
		scriptOp.par.Presencerise = PRESENCE_RISE_FRAMES
		p = page.appendFloat('Presencefall', label='Presence Fall Frames', size=1)
		p[0].default = PRESENCE_FALL_FRAMES
		p[0].min = 1.0
		p[0].max = 90.0
		p[0].clampMin = True
		p[0].clampMax = False
		p[0].help = object_tracker.par_help('Presencefall', subject='person')
		scriptOp.par.Presencefall = PRESENCE_FALL_FRAMES
	...
	def postprocess(self, outputs):
		...
		self.tracked_objects = []
		presence_rise = self._par_or_default('Presencerise', PRESENCE_RISE_FRAMES)
		presence_fall = self._par_or_default('Presencefall', PRESENCE_FALL_FRAMES)
		for t in active_tracks:
			# t.confirmed gates display: a track still in its min_hits confirmation
			# window is genuinely "alive" (kept in active_ids, keeps its tracker-side
			# Kalman/box/mask state) but not yet trusted as a real person.
			#
			# Deliberately NOT also gating on raw t.score here (this used to also skip
			# frames where t.score < self.conf_threshold): that check reacted to genuinely
			# noisy per-frame detection confidence even while the track itself was
			# perfectly stable (lost_frames == 0 every frame), producing visible flicker
			# on an otherwise well-tracked person. presence_ramp() below, driven by
			# lost_frames instead of the noisy raw score, is the smooth replacement --
			# see object_tracker.presence_ramp()'s docstring.
			if not t.confirmed:
				continue
			presence = object_tracker.presence_ramp(
				self._presence_state, t.track_id, t.lost_frames == 0, presence_rise, presence_fall,
			)
		...
		# Prune box/mask/presence state for tracks the tracker has dropped entirely.
		...
		for obj in self.tracked_objects:
			tbl.appendRow([
				...
				*object_tracker.box_row(obj),
				f"{obj['mask_area_ratio']:.4f}", f"{obj['presence']:.4f}",
