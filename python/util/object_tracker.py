...
_PAR_HELP_TEMPLATES = {
	...
	'Presencerise': (
		"Frames to linearly ramp a {subject}'s Presence value from 0 to 1 after it's first "
		"freshly detected -- drives Presence in table_output, for scaling an effect in/out "
		"smoothly instead of a hard on/off cut."
	),
	'Presencefall': (
		"Frames to linearly ramp Presence back down to 0 once a {subject} stops being freshly "
		"detected (independent of Track Loss Frames, which governs how long the tracker keeps "
		"hunting for a re-match before dropping the track entirely -- keep this at or below "
		"Track Loss Frames so Presence reaches 0 before the track itself is pruned)."
	),
}
...
def presence_ramp(state_dict, track_id, is_present, rise_frames, fall_frames):
	"""Linearly-ramping [0, 1] "presence" scalar per track_id, for driving a fade-in/out
	on some downstream effect (scale, opacity, whatever) -- rises by 1/rise_frames each
	call while is_present is True, falls by 1/fall_frames each call while False, clamped
	to [0, 1]. Created at 0.0 on first sight (state_dict.get default), so a brand-new
	track ramps up from nothing rather than snapping straight to 1.

	Deliberately independent of the tracker's own confirmed/track_buffer gating: callers
	should drive is_present from lost_frames == 0 (this frame had a real, fresh-matched
	detection), NOT from a score/confirmed check -- the whole point is a smooth signal
	that isn't reactive to the same frame-to-frame raw-score noise that makes a hard
	score-threshold display gate flicker even while the underlying track is perfectly
	stable (confirmed once, never unset; lost_frames only increments on a genuine
	unmatched frame). rise_frames/fall_frames are independent of Tracklossframes/
	track_buffer on purpose too -- how long a lost object should keep visually fading out
	is a different question from how long the tracker should keep hunting for a
	re-match before dropping the track ID entirely, and the former is usually much
	shorter than the latter.

	Call once per frame per currently-active track_id (including ones with lost_frames >
	0 -- don't skip lost tracks, that's what drives the fall side of the ramp); prune
	entries for tracks that have left active_ids the same way box_smooth()'s state dict
	is pruned, via prune_stale()."""
	current = state_dict.get(track_id, 0.0)
	step = 1.0 / max(rise_frames if is_present else fall_frames, 1)
	current = current + step if is_present else current - step
	state_dict[track_id] = current = max(0.0, min(1.0, current))
	return current
