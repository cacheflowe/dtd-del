...
def providers(gpu_mem_limit_bytes=None):
	...
	NOTE: TensorRT EP was tried here twice and reverted both times -- crashes TD's own
	process outright (no catchable Python exception). Second attempt auto-detected TRT
	via `ort.get_available_providers()` with no par/toggle involved (so the earlier
	THREAD CONFLICT bug is ruled out as the cause) and still crashed -- confirmed live:
	the crash happens INSIDE `ort.get_available_providers()` itself, before a
	TensorRT session is ever created and before any of this function's own log lines
	print. That call must dlopen/probe the provider-shared libraries, which is exactly
	where TD's own bundled onnxruntime DLLs (see providers()'s -- now this function's --
	git history, or docs/learnings/onnx-runtime.md) collide with the pip-installed
	ones. TensorRT is standalone-only (python/standalone/webcam_yolo_pose.py) until/
	unless that's solved via a genuinely separate process, not just avoiding a toggle.
	"""
	cuda_options = {
		'device_id': 0,
