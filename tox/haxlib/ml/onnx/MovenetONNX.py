...
class MovenetONNX:
	...
	def _loadModelThread(self):
		...
		try:
			...
			# Build paths & config
			model_path = os.path.join(project.folder, 'data', 'ml', 'movenet', 'movenet-multipose-lightning.onnx')
	...
	def trackSkeletons(self):
		...
		# Use a tighter threshold than maxMatchDist: true duplicates will be
		# nearly overlapping, while distinct people nearby should still spawn.
		dupThreshold = maxMatchDist * 0.5
